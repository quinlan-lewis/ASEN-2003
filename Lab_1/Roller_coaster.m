...
L_Helixin=sqrt((xlp^2)+(ylp^2)+(zlp^2));
...
L_Helix=sqrt((xLp^2)+(yLp^2)+(zLp^2));
...
L_Helixout=sqrt((xlop^2)+(ylop^2)+(zlop^2));
...
L_Cout= sqrt((xlep^2)+(ylep^2)+(zlep^2));
...
L_parabolic=sqrt((xhp^2)+(yhp^2)+(zhp^2));
...
L_Cvalley=sqrt((xvp^2)+(yvp^2)+(zvp^2));
...
L_Chill=sqrt((xhcp^2)+(yhcp^2)+(zhcp^2));
...
L_Chill2=sqrt((xHcp^2)+(yHcp^2)+(zHcp^2));
...
%            lalong(j,i) = 0;
%        else
%            lalong(j,i) = lalong(j-1,i) + double(subs(Track_path(i,6),s,stepsize(j)));
%        end
