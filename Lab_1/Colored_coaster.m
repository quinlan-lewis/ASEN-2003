%% ASEN 2003 Lab 1
%authors: Quinn Lewis, David Shaw, Noah Hedgepeth, Benjamin Gavin

clear 
clc
close all

%defining all of our beginning vectors
pos_i = [-20,0.9962,125];
accel_i = [0,0,0];
vel_i = [0,0,0];

%% Path Functions
%below are all of the sections of the roller coaster with their respective
%titles at the start of each new block

%We decided to make the x y and z all seperate symbolic functions

%Along with the actual function for the x y and z coordinate is a seperate
%function underneath that was used to find the total length of the track
...
%% Concatenating Matricies & Plotting

%The below commented out section was initially used to graph the total
%track before we changed it to be graphed using the x y and z coordinates.
%Previously we used the fplot3 function which used symbolics to plot
%instead but this proved difficult when we were trying to calculate the
%G loads on the track
%{
hold on
...
%the below portion puts all of the needed data  of the symbolic functions
%and their respective bounds into a matrix. This way it was easier to sort
%through and all of the needed data was in a neat and available location
...
%Below is how we changed all of the data from symbolic functions to actual
%values by using the subs function for the symbolic equations
xplot = zeros(383,11);
zplot = zeros(383,11);
yplot = zeros(383,11);
lalong = zeros(383,11);
steptracker = zeros(11,488);
for i = 1:11
    stepsize = Track_path(i,4):.1:Track_path(i,5);
    
    for j=1:length(stepsize)
       steptracker(i,j) = stepsize(j);
       ...
       if i==1
           lalong(j,i) = (double(steptracker(i,j)) - double(Track_path(i,4)));
       else
           lalong(j,i) = (double(steptracker(i,j)) - double(Track_path(i,4))) + lalong(nnz(steptracker(i-1,:)),i-1);
       end
    end
end

%transposing the arrays made it easier to concatinate the data
xplot = xplot';
yplot = yplot';
zplot = zplot';
lalong = lalong';

%concatinating all of the data into single arrays for easier plotting and
%use
...
%calculates the speed at each point on the track
velreal=zeros(1,length(zplot2));
for i = 1:length(zplot2)
    velreal(i) = velocity(zplot2(i));
end
%plots the data and gives the appropriate titles and axis
figure
hold on
...
%% Calculating Gs
%below is how we calculated the G loads for each section of the track using
%the given equations in the slides presentation

%each portion of the track used somewhat different equations based on the
%shape of the track at that point and the length of that part of the track

rho = (((1+(-9.81*s+25.112)/10.4014)^2)^(3/2))/(abs(9.81/10.4014));
Gdrop = zeros(1,296);
...
%below is concatinating all of the G loading data and then plotting it with
%respect to the distance along the path of the track
